...
df = oskar.load("oskar-spark/src/test/resources/platinum_chr22.small.avro") # type: DataFrame
...
df.withColumn("chunk", col("chromosome").append(col("chromosome"))).show()
...
df.withColumn("freqs", popFreqAsMap("annotation")).show(100)
df.withColumn("freqs", popFreqAsMap("annotation")).where("freqs['1kG_phase3:ALL'] > 0").show(100)
...
df.select(info("studies", 'platinum-genomes-vcf-NA12877_S1.genome.vcf.gz', 'HaplotypeScore')).show(truncate=False)
...
df.select("qual(studies, 'platinum-genomes-vcf-NA12877_S1.genome.vcf.gz')").show(10)
df.select(filter("studies", "platinum-genomes-vcf-NA12877_S1.genome.vcf.gz")).show(truncate=False)

df.select(sample("studies", "NA12877").alias("NA12877"), sample("studies", "NA12878").alias("NA12878")).show(10)
